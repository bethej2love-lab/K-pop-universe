...
// 붙어 ~90분 걸렸는데, Node는 그 제한이 없어 동시 CONC개(기본 50)로 훨씬 빠르다.
//
...
// 옵션 env: MAX_PROMOTE(이번 실행 상한, 기본 무제한) · CONC(동시 프로브, 기본 50) · CHUNK(기본 500)
...
async function isPortrait(id) {
  try {
    ...
    if (!r.ok) return false; // oardefault 없음 = 가로
...
async function main() {
  ...
  while (scanned < MAX) {
    ...
    if (portrait.length) await patchByIds(portrait, { is_short: true, short_probed_at: new Date().toISOString() });
    // 이 청크 전체를 "확인함"으로 표식(세로는 위에서 이미 채웠지만 한 번 더 덮어도 무해)
    await patchByIds(ids, { short_probed_at: new Date().toISOString() });
    scanned += ids.length;

...
//
// 상한 계산(최악의 경우, 2026-09-23 재조정 반영):
//   17~23시(7h) ÷ 48분 ≤ 9 · 00시(1h) ÷ 60분 ≤ 1 · 09~16시(8h) ÷ 60분 ≤ 8 · 01~08시: 무동기화(0)
//   합계 ≤ 18회/일. 동기화 실제 단가는 조회수 갱신을 뺀 280유닛(아래 재배분 절 참고) 기준
//   18×280 = 5,040 + daily-routine 2,800 + 백필 예약 1,500 = 9,340 < 10,000(95% 캡 9,500). ✅
//   여유 160 — `tests/sync-gate.test.js`가 이 상한을 고정해서 "간격을 더 줄이자"가 조용히
//   쿼터 초과로 이어지는 걸 막는다.
//
// ── 2026-09-17 재배분 ────────────────────────────────────────────────────────
...
//
// ── 2026-09-23 재조정: "업로드량"이 아니라 "이용 습관" 기준으로 09~16시를 90→60분으로 ──────
// 사용자 지적: 09~16시 업로드가 적다는 이유로 90분(최대 1.5시간) 간격을 준 게, 실제로는 "직장/학교
// 짬시간에 앱을 열었는데 갱신이 없으면 인스타·유튜브·X로 가버린다"는 체감 이탈을 놓친 설계였다.
// 업로드량 기준 최적화는 "쿼터 대비 새 영상 적재 효율"엔 맞지만, "사용자가 앱을 열 이유"는 다른
// 축이다 — 집중 시간대(09~16시)엔 새 영상이 적어도 최소 매시간은 갱신돼 있어야 한다.
// 예산을 맞추려 저녁 피크만 45→48분으로 3분 완화(66% 구간이라 체감 차이 거의 없음), 그 여유를
// 낮 시간대에 몰아 90→60분(정확히 매시간)으로. 재계산은 아래 "상한 계산" 참고,
// `tests/sync-gate.test.js`(95% 캡)로 고정됨.
function minGapMin(kstHour) {
  if (kstHour >= 17 && kstHour <= 23) return 48;  // 저녁 피크 — 전체 업로드의 66%(45→48, 낮 예산 확보)
  if (kstHour === 0) return 60;                   // 자정 직후에도 87건 — 한 시간 간격은 유지
  if (kstHour >= 9 && kstHour <= 16) return 60;   // 낮 — 집중 시간대, 매시간 갱신(2026-09-23: 90→60)

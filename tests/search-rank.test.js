...
async function main() {
  ...
  try {
    ...
    //    섹션은 점수가 가장 좋은 타입부터 쌓이므로, 접두일치 프로그램 하나가 섹션을 앞으로 끌어오면
    //    같은 섹션의 부분일치 프로그램(라디오스타)까지 곡 위로 딸려 올라간다.
    //    (그 접두일치 프로그램이던 STAR ZOOM IN은 2026-09-11에 목록에서 빠졌다 — 아래 접두일치 검사는
    //     원래 조건부라 해당 프로그램이 없으면 그냥 건너뛴다.)

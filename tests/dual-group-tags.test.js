...
// ⚠️ 범위(2026-09-16 확장) — 기준을 "이 영상 members에 같은 이름"에서 **"그 group_ko 로스터에 같은 이름"**
//    으로 넓혔다. 좁은 조건은 **이 영상이 홈 멤버를 태깅했을 때만** 발동해서, 미태깅 영상에서 가드가
//    통째로 비켜갔다 — 실측(2026-09-16): 엔시티 127 채널의 `마크(갓세븐)` 오태깅 3건이 전부 members:[]라
//    살아남았다(`맛피아 트레이드 마크 수준` 같은 순수 오탐 포함).
//    전수 시뮬레이션 12,000건에서 순증분 41건(엑스원 김우석↔판타지보이즈 16 · 모모랜드 연우↔나우즈 6 ·
//    오메가엑스 정훈↔싸이커스 4 · 엔시티127 마크↔갓세븐 3 …)이고 표본 20건 전부 정당한 제거였다.
t('members가 비어도 로스터에 같은 이름이 있으면 제거(실측 마크 유형)',
  N({title:'아무 제목',groupKo:'세븐틴',members:[],withMembers:['민규(동키즈)']}),[]);
t('로스터에 그 이름이 없으면 손대지 않는다(근거를 못 잰 구간)',
  N({title:'아무 제목',groupKo:'세븐틴',members:['호시'],withMembers:['유주(체리블렛)']}),['유주(체리블렛)']);
